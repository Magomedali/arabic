<?php
namespace backend\modules\rbac\models\forms;
...
class AssignmentForm extends Model
{
    ...
    {
        return [
            'assigned' => Yii::t('rbac', 'RBAC_ASSIGNED'),
            'unassigned' => Yii::t('rbac', 'RBAC_UNASSIGNED'),
        ];
    }
    ...
    public function noEmpty($attribute, $parameters)
    {
        if ($this->action == $parameters['action']) {
            ...
            if (!is_array($items) || !count($items)) {
                $this->addError($attribute, Yii::t('rbac', 'CHOOSE_ITEMS'));
            }
        }
    }
    ...
    protected function saveAssignments($items, $method)
    {
        ...
        foreach ($items as $item) {
            $allSuccess = $allSuccess && $auth->$method($this->getItem($item), $this->model->user->id);
        }
