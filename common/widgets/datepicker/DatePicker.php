...
class DatePicker extends InputWidget
{
    ...
    protected $script_prefix = "dt_";


    protected static $jsScripts = array();

    protected $jsFuncionUpdate = "";

    public $lastWidgetInPage = false;
    ...
    public function run()
    {
        ...
        $this->registerJsIntoScripts();

        if($this->lastWidgetInPage){
            $this->registerWidgetClass();  
            $this->registerJsIntoFunction();
        }
    ...
    public function registerAssets()
    {
        ...
        $prefix = count(self::$jsScripts);

        
        $view->registerJs('jQuery.fn.kvDatepicker.dates={};',$view::POS_READY,$this->script_prefix.++$prefix);
        ...
        $merge = $this->getMergePreAndPast($pre,$post);
        if(count($merge)){
            foreach ($merge as $key => $value) {
                $this->setJsScripts($this->script_prefix.++$prefix,$value);
            }
        }

        if ($this->type === self::TYPE_INLINE) {
            $view->registerJs("{$el}.on('changeDate',function(e){{$input}.val(e.format()).trigger('change')});",$view::POS_READY,$this->script_prefix.++$prefix);
        }
        if ($this->_hasAddon && $this->removeButton !== false) {
            $view->registerJs("initDPRemove('{$id}');",$view::POS_READY,$this->script_prefix.++$prefix);
        }
        if ($this->_hasAddon) {
            $view->registerJs("initDPAddon('{$id}');",$view::POS_READY,$this->script_prefix.++$prefix);
        }
        if ($this->type === self::TYPE_RANGE) {
            
            FieldRangeAsset::register($view);

            $view->registerJs("initDPRemove('{$id}', true);",$view::POS_READY,$this->script_prefix.++$prefix);
    ...
    public function registerJsIntoScripts(){
        $view = $this->getView();
        $scripts = $view->js;
        if(is_array($scripts) && count($scripts)){
            //self::$jsScripts = array_merge($scripts,self::$jsScripts);
            foreach ($scripts as $key1 => $s) {
                if(is_array($s)){
                    foreach ($s as $key2 => $value) {
                        if(preg_match("/$this->script_prefix/i", $key2)){
                            self::$jsScripts[$key2] = $value;
                        }
                    }
                }else{
                    if(preg_match("/$this->script_prefix/i", $key1)){
                        self::$jsScripts[$key1] = $s;
                    } 
                }
            }
            ksort(self::$jsScripts);
        }
    }


    public function registerJsIntoFunction(){
        $view = $this->getView();

        if(count(self::$jsScripts)){
            $script = "
                if(typeof ALDT == 'object'){ ALDT.updateCallBack = function(){\n\r";
            
            foreach (self::$jsScripts as $key => $s) {
                $script .= "\n\t\t".$s;
                $script .= "\n\t\t//---------------------------";
    ...
    protected function registerWidgetClass(){
         $view = $this->getView();
         $script = <<<JS

         var AlDatePicker = function(opt = {}){

            this.updateCallBack = opt.updateCallBack;
...
JS;
    ...
    public function setJsScripts($key,$scripts){
        
        self::$jsScripts[$key] = $scripts;
    }
    ...
    public function getMergePreAndPast($pre,$past){
        $merge = [];
        if(is_array($pre) && is_array($past)){
