...
/**
 * Class m171208_115012_rbac
 */
class m171208_115012_rbac extends Migration
{   
    ...
    public function addData(){
        ...
        if(isset($data['auth_item']) && is_array($data['auth_item'])){
            $authManager = $this->getAuthManager();
            foreach ($data['auth_item'] as $key => $role) {
            ...
            $this->insert($authManager->assignmentTable,[
                'item_name'=>$data['auth_item'][0]['name'],
                'user_id'=>1,
                'created_at'=>time()
            ]);
